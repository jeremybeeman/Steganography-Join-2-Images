...
startTime = time.time()
print("encoding...")
[imgEncoded, unusedBits, imgBehindShape] = b_join.simple_bitwise_4X1_encode(front_image_path, back_image_path, numBitPlanes)
print("done encoding. Time (seconds):", time.time()-startTime)
print("Bottom image shape:", imgBehindShape)
cv2.imwrite(joined_image_path, imgEncoded)

###Decoding section 
startTime = time.time()
print("decoding...")
imgsDecoded = b_join.simple_bitwise_4X1_decode(joined_image_path, unusedBits, imgBehindShape, numBitPlanes)
